...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    "melding a heap containing two low values with a heap containing two high values" ->
      forAll { (x: Int, y: Int) =>
        ...
        val s = if (x < y) x else y
        val l = if (x < y) y else x
        val meldedHeap: List[Node] =
          meld(insert(l, insert(l, empty)), insert(s, insert(s, empty)))
        ...
        val deleteTwoMinAndFindMin: Boolean =
          l == findMin(deleteMin(deleteMin(meldedHeap)))
  ...
  val meldingHeaps: (String, Prop) =
    def meldingHeapsCheck(
        heap1: List[Node],
        heap2: List[Node],
        meldedHeap: List[Node]
    ): Boolean =
      if isEmpty(heap1) && isEmpty(heap2) && isEmpty(meldedHeap) then true
      else if !isEmpty(heap1) && findMin(heap1) == findMin(meldedHeap) then
        meldingHeapsCheck(deleteMin(heap1), heap2, deleteMin(meldedHeap))
      else if !isEmpty(heap2) && findMin(heap2) == findMin(meldedHeap) then
        meldingHeapsCheck(deleteMin(heap2), heap1, deleteMin(meldedHeap))
      else false
    "finding the minimum of melding any two heaps should return the minimum of one or the other of the source heaps" ->
      forAll { (heap1: List[Node], heap2: List[Node]) =>
        meldingHeapsCheck(heap1, heap2, meld(heap1, heap2))
      }
